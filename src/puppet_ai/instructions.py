...
MCP_INSTRUCTIONS = """puppet-ai: Human-like computer perception and control for AI agents.
...
Before using puppet-ai for a task, ask: what's the fastest way?

PRIORITY ORDER (try in this order):
1. API/WebFetch — if the site has an API or page is fetchable (fastest)
2. puppet-ai — if API is blocked, requires auth, or it's a desktop app (reliable, works everywhere)

USE puppet-ai (eyes + hands) for:
...
2. If WebFetch fails (403, blocked, empty, login required) → switch to puppet-ai
3. puppet-ai ALWAYS works — it sees exactly what the user sees in their browser
